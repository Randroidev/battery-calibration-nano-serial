...
static void print_detailed_status_flags(uint16_t status_word) {
  ...
  ui_print_param(F("Is Charging"), (status_word & 0x0040) ? "YES" : "NO");
  ui_print_param(F("Is Discharging"), (status_word & 0x0080) ? "YES" : "NO");
  ...
  ui_print_param(F("Terminate Charge Alarm (TCA)"), (status_word & 0x0800) ? "SET" : "CLEAR");
  ui_print_param(F("Over Temp Alarm (OTA)"), (status_word & 0x0400) ? "SET" : "CLEAR");
  ui_print_param(F("Remaining Time Alarm (RTA)"), (status_word & 0x0200) ? "SET" : "CLEAR");
  ui_print_param(F("Remaining Capacity Alarm (RCA)"), (status_word & 0x0100) ? "SET" : "CLEAR");
  ui_print_message(F("  =================================="));
}
